...
def conform(base: str) -> None:
    ...
    check(
        "GET /tables lists the warehouse's tables",
        st == 200 and "dbo.fct_revenue_summary" in tables,
        f"{len(tables)} tables",
    )
    ...
    check(
        "GET /tables/{name} reports columns and types",
        st == 200 and "revenue_usd" in cols and cols["revenue_usd"]["type"].startswith("decimal"),
        cols.get("revenue_usd", {}).get("type", ""),
    )

    st, body = ex.rest(
        "POST", "/query", carol, {"sql": "SELECT COUNT(*) AS n FROM dbo.fct_revenue_summary"}
    )
    check(
        "POST /query returns rows, the statement run, and the tables read",
        st == 200
        and body.get("rowCount") == 1
        and body.get("tables") == ["dbo.fct_revenue_summary"]
    ...
    check(
        "the row ceiling is applied and reported",
        st == 200
        and body.get("rowCount") == int(c.CFG.get("DAS_SQL_MAX_ROWS", "500"))
        and body.get("truncated") is True,
        f"{body.get('rowCount')} rows",
    )

    st, body = ex.rest(
        "POST", "/query", carol, {"sql": "SELECT * FROM dbo.dim_product", "maxRows": 3}
    )
    check(
        "a smaller caller ceiling is honoured",
        st == 200 and body.get("rowCount") == 3,
        f"{body.get('rowCount')} rows",
    )
    ...
    check(
        "MCP initialize answers with server info",
        st == 200 and bool(info.get("name")),
        info.get("name", ""),
    )
    ...
    check(
        "run_query's schema requires sql and forbids extras",
    ...
    check(
        ...
        st == 200 and "error" in payload,
        str(payload.get("error", {}).get("message", ""))[:60],
    )

    err, text = ex.tool("run_query", {"sql": "SELECT COUNT(*) AS n FROM dbo.dim_product"}, carol)
    check(
        "a tool call returns the same payload as the REST route",
        err is False and json.loads(text).get("rowCount") == 1,
        text[:60],
    )
    ...
    check(
        "every refused statement is refused for the stated reason",
        refused == len(REFUSED),
        f"{refused}/{len(REFUSED)}" + (f"; {wrong[:2]}" if wrong else ""),
    )
    ...
    check(
        "every permitted statement is permitted",
        allowed_ok == len(ALLOWED),
        f"{allowed_ok}/{len(ALLOWED)}",
    )

    # ---- authorization ------------------------------------------------
    err, text = ex.tool(
        "run_query", {"sql": "SELECT customer_id, email FROM dbo.dim_customer"}, alice
    )
    check(
        "a role is refused a column it may not read",
    ...
    check(
        "withheld columns are not described",
        err is False
        and "email" not in [c_["name"] for c_ in described.get("columns", [])]
    ...
    check(
        "every source declares how its callers are authorized",
    ...
    if user_tier:
        ...
        check(
            f"a user-tier source ({name}) answers only after acting for the caller",
            err is False,
            text[:60],
        )
        err, text = ex.tool("run_query", {"source": name, "sql": SIMPLE_COUNT}, bob)
        check(
            f"and refuses a caller the SOURCE does not know ({name})",
            bool(err) and "access" in text.lower(),
            text[:70],
        )
    ...
    if service_tier:
        ...
        check(
            f"a service-tier source ({name}) cannot distinguish its callers",
    ...
    check(
        "protected-resource metadata is served (RFC 9728)",
        meta.get("resource") == c.CFG["DAS_AGENT_AUDIENCE"]
        and bool(meta.get("authorization_servers")),
        meta.get("resource", ""),
    )
    ...
    check(
        "the metadata states the scope and that registration is not available",
        meta.get("scopes_supported")
        == [(f"{c.CFG['DAS_AGENT_AUDIENCE']}/{c.CFG.get('DAS_REQUIRED_SCOPE', 'access_as_user')}")]
