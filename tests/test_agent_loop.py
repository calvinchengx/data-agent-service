...
def final(text):
    return Response(stop_reason="end_turn", content=[Block(type="text", text=text)],
...
def test_runs_a_tool_then_answers(patched):
    ...
    patched(toolbox)
    client = ScriptedClient([tool_use("t1", "warehouse__run_query", {"sql": "SELECT 1"}),
                             final("One row.")])
...
def test_tool_result_is_returned_with_its_id(patched):
    ...
    patched(toolbox)
    client = ScriptedClient([tool_use("abc", "warehouse__run_query", {"sql": "SELECT 1"}),
                             final("done")])
...
def test_a_refusal_reaches_the_model_as_a_result(patched):
    ...
    patched(toolbox)
    client = ScriptedClient([tool_use("t1", "warehouse__run_query", {"sql": "DROP TABLE x"}),
...
def test_gives_up_rather_than_looping_forever(patched, monkeypatch):
    ...
    patched(toolbox)
    client = ScriptedClient([tool_use(f"t{i}", "warehouse__run_query", {"sql": "SELECT 1"})
                             for i in range(3)])
    answer = agent_mod.ask("q", "token", client=client)
...
def test_a_model_refusal_is_reported_not_raised(patched):
    toolbox = FakeToolbox([])
    patched(toolbox)
    client = ScriptedClient([Response(stop_reason="refusal", content=[],
