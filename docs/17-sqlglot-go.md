...
| **B5** the remaining 29 dialects | **spiked, not started** | one bounded spike (Redshift, the cheapest -- a thin Postgres subclass) reached 18.8% exact match from pure mechanical table generation alone, 30.9% after the single highest-leverage fix (a dialect flag stamping every Column with `join_mark`); the rest of its ~174 remaining mismatches were still a long tail of unrelated causes (type names, time-format strings, operators, function quirks, generator gaps) when the spike was reverted. Confirms each dialect is real, independent, multi-session work -- not something to batch through |
| **B6** `executor`, `planner`, `lineage`, `diff`, `jsonpath`, `anonymize` | **started** -- `jsonpath` and `anonymize` done | `jsonpath` rewritten from a partial hand-rolled scanner (3 of 8 grammar node kinds) into a real tokenizer/recursive-descent parser, held to the reference's own JSONPath Compliance Test Suite (526 cases): 519 agreed, 0 wrong, 7 declined (Python's unbounded ints overflowing Go's). Also fixed a real pre-existing bug it surfaced: DuckDB's `->` operator was missing a JSON-Pointer guard its sibling call site already had. `anonymize` ported as token-level (not AST) text scrubbing -- fixed-width, length-preserving, consistent aliases over identifiers/strings/numbers, comment and hint bodies blanked -- measured over the port's own 4,508-statement corpus at 4508/4508, 0 wrong, plus a hand-written suite lifted from the reference's own tests for what that valid-SQL corpus cannot reach (hints, unterminated input, huge numbers). `executor`/`planner` (an in-process SQL engine) fit this port's purpose less well than `lineage`/`diff`, which are what remain unstarted |
...
| 3 / Target B — the rest of sqlglot | ~30,000 | multi-quarter | **oracle done; B4 effectively done (466/480); B1 effectively done (104/113); B3 effectively done (gap report 19 -> 7); B5 spiked (Redshift: 18.8% -> 30.9%, reverted); B6 started (jsonpath 519/526 CTS, anonymize 4508/4508, both 0 wrong)** |
