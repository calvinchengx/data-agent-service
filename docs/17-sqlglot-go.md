...
| **B6** `executor`, `planner`, `lineage`, `diff`, `jsonpath`, `anonymize` | **started** -- `jsonpath` done | rewritten from a partial hand-rolled scanner (3 of 8 grammar node kinds) into a real tokenizer/recursive-descent parser, held to the reference's own JSONPath Compliance Test Suite (526 cases): 519 agreed, 0 wrong, 7 declined (Python's unbounded ints overflowing Go's). Also fixed a real pre-existing bug it surfaced: DuckDB's `->` operator was missing a JSON-Pointer guard its sibling call site already had. `executor`/`planner` (an in-process SQL engine) fit this port's purpose less well than `anonymize`/`lineage`/`diff`; none of the other four are started |
...
| 3 / Target B — the rest of sqlglot | ~30,000 | multi-quarter | **oracle done; B4 effectively done (466/480); B1 effectively done (104/113); B3 effectively done (gap report 19 -> 7); B5 spiked (Redshift: 18.8% -> 30.9%, reverted); B6 started (jsonpath: 519/526 CTS, 0 wrong)** |
