...
def protocol(tok: str) -> None:
    ...
    got = r.get("result", {}).get("protocolVersion")
    check(
        "an unknown version gets a supported one rather than an echo",
        st == 200 and got in SUPPORTED,
        str(got),
    )
    ...
    check("a notification is accepted with no body", st == 202 and not b.strip(), f"status {st}")

    st, r = rpc("no/such/method", {}, tok)
    check(
        "an unknown method is -32601",
        r.get("error", {}).get("code") == -32601,
        str(r.get("error", {}).get("code")),
    )

    st, r = rpc("tools/call", {"name": "no_such_tool", "arguments": {}}, tok)
    check(
        "an unknown tool is -32602 (a protocol error, not a tool result)",
    ...
    check(
        "the unused server-to-client stream is declined, not left open",
        st in (405, 404),
        f"status {st}",
    )
    ...
    check(
        "every tool schema is a plain JSON Schema object",
        not schema_problems,
        "; ".join(schema_problems)[:120],
    )
    check(
        "no vendor-specific fields on any tool", not vendor_fields, ", ".join(vendor_fields)[:120]
    )
...
def discovery(tok: str) -> None:
    ...
    check(
        "an unauthenticated call is refused with a challenge",
    ...
    check(
        "the URL in the challenge actually serves the metadata",
        st == 200 and meta.get("resource") == AUD,
        f"{st} {advertised.group(1) if advertised else 'no resource_metadata'}"[:100],
    )

    check(
        "protected-resource metadata names this resource and its authorization server",
        meta.get("resource") == AUD and bool(meta.get("authorization_servers")),
        meta.get("authorization_servers", [""])[0] if meta else f"status {st}",
    )
    check(
        "the scope a client must ask for is stated",
        bool(meta.get("scopes_supported")),
        ", ".join(meta.get("scopes_supported", [])),
    )
    ...
    check(
        "the authorization server's own metadata is reachable from it",
        st == 200 and bool(as_meta.get("token_endpoint")),
        as_meta.get("issuer", "")[:70],
    )
    ...
    check(
        "the resource server does not restate the authorization server's metadata",
        st_copy in (404, 405),
        f"status {st_copy}",
    )
    check(
        "the authorization server offers the flows an interactive client needs",
    ...
    check(
        "the authorization server does not advertise registration it lacks",
    ...
    check(
        "every field is either RFC 9728 or pinned by the executor contract",
        not unknown,
        ", ".join(unknown)
        or f"{len(set(meta) & rfc9728)} standard + {len(set(meta) & pinned)} pinned",
    )
    ...
    async with http_client, streamable_http_client(URL, http_client=http_client) as streams:
        ...
        async with ClientSession(read, write) as session:
            ...
            result = await session.call_tool(
                "run_query", {"sql": "SELECT COUNT(*) AS n FROM dbo.fct_revenue_summary"}
            )
            out["is_error"] = bool(result.is_error)
            out["text"] = "".join(getattr(b, "text", "") for b in result.content)[:200]
            refused = await session.call_tool("run_query", {"sql": "DROP TABLE dbo.fct_sales"})
            out["refused"] = bool(refused.is_error)
            out["refusal_text"] = "".join(getattr(b, "text", "") for b in refused.content)[:120]
    return out
...
def reference_client(tok: str) -> None:
    ...
    try:
        ...
        return
    check(
        "the Python SDK completes a session",
        bool(out.get("server")),
        f"{out.get('server')} @ {out.get('version')}",
    )
    check(
        "it lists the same tools",
        "run_query" in out.get("tools", []),
        ", ".join(out.get("tools", [])),
    )
    check("it runs a query and gets rows", out.get("is_error") is False, out.get("text", "")[:80])
    check(
        "a refusal reaches it as a tool error it can show the user",
        out.get("refused") is True,
        out.get("refusal_text", "")[:80],
    )
...
def reference_client_typescript(tok: str) -> None:
    ...
    if not (JS_HOME / "node_modules").exists():
        check(
            "the TypeScript SDK completes a session",
            False,
    ...
    check(
        "the TypeScript SDK completes a session, lists the tools, and sees the guard refuse",
        proc.returncode == 0,
