...
def settings(cfg: dict, env: str) -> None:
    ...
    if env == "prod":
        ...
        check(
            "settings that must be empty in production are empty",
            not filled,
        ...
        check(
            ...
            validating,
            "DAS_APIM_VALIDATE_JWT is off; it is only off locally (docs/upstream-issues.md #7)",
        )
...
def identity(cfg: dict) -> None:
    ...
    check(
        "the tenant answers",
        st == 200 and meta.get("issuer") == issuer,
    ...
    check(
        "it offers the flows an interactive client needs",
...
def reachability(cfg: dict) -> None:
    ...
    check("the gateway is serving and refuses an anonymous call", st in (401, 403), f"status {st}")
    check(
        "it tells a client where to authenticate",
